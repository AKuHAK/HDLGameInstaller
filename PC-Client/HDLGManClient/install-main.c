				...
				for(lsn=0; lsn<sectorcount+dl_dvd_sectors; lsn+=SectorsToRead){
					...
					for(retries = 0; retries < RETRY_COUNT; retries++)
					{
						...
						else
						{
							...
							if(result == 0)
							{	//All OK. Only the data connection was disconnected.
							}
							...
							{	//Some other error, like disk I/O error (non-recoverable).
							...
							if(result == 0)
							{	//Command connection is okay. Attempt to reconnect for data connection.
								for (reconnects = 0; reconnects < RECONNECT_COUNT; reconnects++)
								{
									if ((result = ReconnectToServerForWrite(DiscID, sectorcount+dl_dvd_sectors-lsn, lsn)) == 0)
									{	//Successfully re-connected.
										break;
									}
								}
...
GameCopy_end:
				...
				if(installResult >= 0)
				{
					...
					else memset(&ConvertedIcon, 0, sizeof(ConvertedIcon));
				...
				for(retries = 0; retries < RETRY_COUNT; retries++)
				{
					...
					else
					{
						...
						{	//Command connection is okay. Attempt to reconnect for data connection.
							for (reconnects = 0; reconnects < RECONNECT_COUNT; reconnects++)
							{
								if ((result = ReconnectToServerForRead(params->partition, params->sectors - lsn, lsn)) == 0)
								{	//Successfully re-connected.
									break;
								}
							}
